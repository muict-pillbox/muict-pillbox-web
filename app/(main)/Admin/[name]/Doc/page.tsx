...
function PatientPage() {
  ...
  useEffect(() => {
    ...
    getUserFromCookie().then(async (_) => {
      ...
      try {
        ...
        const data = await graphQLClient.request<{ Userinfo: UserInfo[] }>(query, { Filter: "HOSPITAL"});
  ...
  function deleteDoctor(event: React.MouseEvent<HTMLButtonElement, MouseEvent>): void {
          ...
          {patientList.map((patient, index) => (
              <div key={index}>
                ...
                <div>
                  ...
                  <button onClick={deleteDoctor}>ลบข้อมูล</button>
                </div>
                )}
              </div>
            ))}
